...
RSpec.describe PublicStorage::Facility do
  ...
  describe '.fetch' do
    ...
    it 'fetches and parses the facility' do
      ...
      expect(fetch.address).to be_a(PublicStorage::Address)
      expect(fetch.geocode).to be_a(PublicStorage::Geocode)
      expect(fetch.prices).to all(be_a(PublicStorage::Price))
    end
  end
end
